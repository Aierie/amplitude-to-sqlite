...
pub struct ExportEvent {
    ...
    pub event_id: Option<i64>,
    pub event_properties: Option<HashMap<String, Value>>,
    #[serde(deserialize_with = "deserialize_export_time", serialize_with = "serialize_export_time")]
...
impl ExportEvent {
    ...
    pub fn to_batch_event(&self) -> Result<Event, Box<dyn std::error::Error>> {
        ...
        Ok(Event {
            ...
            app_version: self.version_name.as_ref().and_then(|v| v.as_str()).map(|s| s.to_string()),
...
mod tests {
    ...
    fn test_export_event_conversion() {
        ...
        assert_eq!(batch_event.user_id, Some("2b59c518-dc2b-43f6-9444-85a84d5b1e1f".to_string()));
        assert_eq!(batch_event.device_id, Some("f48913e6-c106-5429-a296-9f9588859c3b".to_string()));
        ...
        assert_eq!(batch_event.insert_id, Some("Property Drop Purchased:2".to_string()));
        ...
        assert_eq!(event_props.get("Price per Share"), Some(&Value::from(312.0)));
        assert_eq!(event_props.get("Property"), Some(&Value::from("Property 0")));
        ...
        assert_eq!(batch_event.time, expected_time, "Timestamps do not match: batch_event.time = {}, expected_time = {}", batch_event.time, expected_time);
    }
    ...
    fn test_round_trip_json_conversion() {
        ...
        for (i, (original_line, export_event)) in original_lines.iter().zip(export_events.iter()).enumerate() {
        ...
        println!("Successfully completed round-trip JSON conversion test for {} events", export_events.len());
    }
} 